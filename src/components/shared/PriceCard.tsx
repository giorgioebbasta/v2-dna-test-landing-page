...
const PriceCard = React.memo(({ name, currentPrice, originalPrice, savings, promoCode, url, onClick, extraDiscount, compact = false, horizontal = false }: PriceCardProps) => {
  ...
  if (horizontal) {
    return (
      <Card 
        className="gtm_addtocart border-2 border-[#fbd5d1] hover:border-[#fbd5d1]/80 transition-all duration-300 hover:shadow-lg cursor-pointer relative overflow-hidden h-full"
        onClick={handleClick}
      >
