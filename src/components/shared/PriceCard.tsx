...
const PriceCard = React.memo(({ name, currentPrice, originalPrice, savings, promoCode, url, onClick, extraDiscount, compact = false, horizontal = false }: PriceCardProps) => {
  ...
  if (horizontal) {
    return (
      <Card 
        className="gtm_addtocart border-2 border-[#f5bcb5] hover:border-[#f5bcb5]/80 transition-all duration-300 hover:shadow-lg cursor-pointer relative overflow-hidden h-full"
        onClick={handleClick}
      >
        <CardContent className="p-3 h-full flex items-center justify-between gap-3">
          ...
          <div className="text-right flex flex-col justify-center">
            ...
            <div className="text-xs font-medium text-[#0B4650] mt-0.5">Risparmi €{savings}</div>
          </div>
        </CardContent>
      </Card>
    );
